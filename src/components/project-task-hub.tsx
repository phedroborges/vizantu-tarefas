...
import { TASK_STATUSES, type Member, type StatusGroup, type Tag as TaskTag, type Task } from "@/lib/types";
...
export function ProjectTaskHub({ tasks: initialTasks, formatTags, channelTags, members, canEdit, initialView = "lista" }: { tasks: Task[]; formatTags: TaskTag[]; channelTags: TaskTag[]; members: Member[]; canEdit: boolean; initialView?: "lista" | "calendario" | "board" }) {
  ...
  async function moveTask(taskId: string, dueDate: string) {
    const previous = tasks.find((task) => task.id === taskId)?.dueDate;
    setTasks((current) => current.map((task) => task.id === taskId ? { ...task, dueDate } : task));
    try {
      const response = await fetch(`/api/tasks/${taskId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ dueDate }) });
      if (!response.ok) throw new Error("Falha ao alterar a data");
      const result = await response.json();
      setTasks((current) => current.map((task) => task.id === taskId ? result.task : task));
    } catch {
      setTasks((current) => current.map((task) => task.id === taskId ? { ...task, dueDate: previous } : task));
    }
  }
  ...
  return <Card className="project-hub">
    ...
    {tasks.length && view === "calendario" ? <PlanCalendar tasks={tasks} formatTags={formatTags} channelTags={channelTags} members={members} canEdit={canEdit} onMove={(taskId, dueDate) => void moveTask(taskId, dueDate)} onOpen={(task) => router.push(`/tarefas/${task.id}`)} /> : null}
  </Card>;
}
