...
import { Bell, CheckCheck, Clock3, FileText, Megaphone, MessageSquareText, UserCheck, Wallet } from "lucide-react";
...
const meta: Record<NotificationType, { label: string; icon: ReactNode }> = {
  mention: { label: "Menção", icon: <MessageSquareText size={17} /> }, task_assigned: { label: "Responsabilidade", icon: <UserCheck size={17} /> }, task_overdue: { label: "Atraso", icon: <Clock3 size={17} /> }, announcement: { label: "Aviso", icon: <Megaphone size={17} /> }, finance_receivable: { label: "Cobrança", icon: <Wallet size={17} /> }, contract_ending: { label: "Contrato", icon: <FileText size={17} /> },
};
