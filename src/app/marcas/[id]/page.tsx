...
export default async function BrandPage({ params }: { params: Promise<{ id: string }> }) {
  const user = await requirePageAccess("marcas");
  ...
  if (!project) notFound();
  return <AdminShell active="marcas" user={user}><BrandDetailView plan={plan} project={project} initialTasks={tasks} members={members} formatTags={formatTags} channelTags={channelTags} categoryTags={categoryTags} statusColors={statusColors} currentUserId={user.id} canEdit /></AdminShell>;
}
