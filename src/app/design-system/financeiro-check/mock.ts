...
for (const [monthIndex, month] of ["2026-06", "2026-07", "2026-08", "2026-09", "2026-10", "2026-11"].entries()) {
  for (const [i, project] of projects.entries()) entries.push({ id: `income-${month}-${i}`, direction: "income", category: "servicos", description: `Gestão de marca · ${project.name}`, amount: 300000 + i*70000 + monthIndex*10000, dueDate: `${month}-10`, competence: month, projectId: project.id, memberId: null, recurring: true, seriesId: project.id, sourceKey: `demo:${month}:${i}`, cancelled: false, notes: "Dados demonstrativos", createdAt: iso });
  for (const [i, category] of ["producao", "operacional", "ferramentas", "prolabore"].entries()) entries.push({ id: `expense-${month}-${i}`, direction:"expense",category: category as Entry["category"],description: ["Produção do mês", "Operação", "Licenças e ferramentas", "Pró-labore"][i],amount:[230000,90000,56000,200000][i],dueDate:`${month}-15`,competence:month,projectId:null,memberId:null,recurring:false,seriesId:null,sourceKey:null,cancelled:false,notes:"",createdAt:iso });
}
export const financeFixture: FinanceData = {
  entries, payments: entries.filter((e) => e.competence < "2026-09" || (e.direction === "income" && e.projectId === projects[0].id && e.competence === "2026-09")).map((entry) => ({ id: `paid-${entry.id}`,entryId:entry.id,amount:entry.amount,paidAt:entry.dueDate,method:"Pix",reference:"Exemplo",reversed:false })),
  settings: { ...DEFAULT_SETTINGS, taxRate:6,taxRegime:"Alíquota demonstrativa",openingBalance:1800000,openingDate:"2026-06-01" },
