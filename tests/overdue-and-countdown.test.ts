...
describe("atraso e contagem regressiva", () => {
  ...
  it("3. toda etapa ainda não entregue pode ficar atrasada", () => {
    for (const status of ["rascunho", "aguardando_informacao", "aprovacao_copy", "aguardando_captacao", "pronto_para_criacao", "em_criacao", "revisao", "ajuste"] as const) {
      expect(overdueDays("2026-01-01", status, HOJE), status).toBeGreaterThan(0);
      expect(isOverdue("2020-01-01", status), status).toBe(true);
    }
  });

  it("4. status entregue ou encerrado nunca conta como atrasado", () => {
    for (const status of ["para_aprovacao", "aprovado", "problema", "finalizado"] as const) {
      expect(overdueDays("2026-01-01", status, HOJE)).toBe(0);
      expect(isOverdue("2020-01-01", status)).toBe(false);
    }
  });

  it("5. sem prazo não é atraso", () => {
    expect(overdueDays(undefined, "em_criacao", HOJE)).toBe(0);
  });

  it("6. o prazo vale até o fim do dia, não até a hora atual", () => {
  ...
  it("7. o rótulo muda de minutos para horas e para dias", () => {
  ...
  it("8. faltando menos de uma hora aparece em minutos", () => {
  ...
  it("9. prazo vencido vira 'atrasada há'", () => {
  ...
  it("10. o contador e a coluna de prazo contam o MESMO atraso", () => {
  ...
  it("11. a ordenação põe a mais atrasada primeiro", () => {
    const tarefas = [
