...
import { loginAsTestDono, loginAsTestCriativo, reloadAndWaitForAnnouncements } from "./helpers";
...
test.describe("Aviso — broadcast com confirmação obrigatória", () => {
  ...
  test("2. escopo 'role' só aparece pra quem tem aquele papel", async ({ page }) => {
    await loginAsTestDono(page);
    await page.request.post("/api/announcements", { data: { body: "[E2E] Aviso pro diretor criativo", scope: "role", scopeRole: "diretor_criativo" } });

    // dono não deveria ver este aviso.
    await reloadAndWaitForAnnouncements(page);
    await expect(page.getByText("[E2E] Aviso pro diretor criativo")).toBeHidden();
  });

  test("3. escopo 'role' aparece pro cargo de verdade (mostrado na fila, um por vez)", async ({ page }) => {
    await loginAsTestCriativo(page);
    const okButton = page.getByRole("button", { name: "Ok, entendi" });
    // O diretor criativo também pode ter o aviso "para todos" do teste 1 ainda
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (await page.getByText("[E2E] Aviso pro diretor criativo").isVisible()) break;
      await okButton.click();
    }
    await expect(page.getByText("[E2E] Aviso pro diretor criativo")).toBeVisible();
    await okButton.click();
    await expect(page.getByText("[E2E] Aviso pro diretor criativo")).toBeHidden();
  });
