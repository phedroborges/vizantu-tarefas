...
const { auth, load, mutate, session, blocked, from } = vi.hoisted(() => ({ auth: vi.fn(), load: vi.fn(), mutate: vi.fn(), session: vi.fn(), blocked: vi.fn(), from: vi.fn() }));
vi.mock("@/lib/authz", () => ({ requireUser: auth, isResponse: (value: unknown) => value instanceof NextResponse }));
vi.mock("@/lib/finance/storage", () => ({ loadFinance: load, mutateFinance: mutate, FinanceInputError: class extends Error {} }));
